...
const DetailBerita = () => {
  ...
  useEffect(() => {
    ...
    const fetchNewsDetail = async () => {
      try {
        ...
        const selectedNews = response.data.find((item) => item.id == id);
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto p-6">
      <img src={newsDetail.foto} alt={newsDetail.judul} className="w-full h-64 object-cover rounded-lg mb-4" />
      <h1 className="text-2xl font-bold mb-2">{newsDetail.judul}</h1>
      <p className="text-gray-600 mb-2">{new Date(newsDetail.tanggal_dibuat).toLocaleDateString()}</p>
